...
#' )
#' }
calculate_pre_and_post_fortification_summaries <- function(
    householdConsumptionDf = householdConsumption,
    householdDetailsDf = householdDetails,
    nctListDf = nctList,
    intakeThresholdsDf = intakeThresholds,
    ...
    supportedMNList <- c("Ca", "Carbohydrates", "Cu", "Energy", "Fat", "Fe", "Fibre", "I", "IP6", "Mg", "Protein", "Se", "Zn", "Ash", "B6", "B2", "D", "N", "K", "P", "Moisture", "Cholesterol", "E", "Na", "A", "C", "B12", "B1", "B3", "B9", "B5", "B7", "Mn")

    # Check if MN is in the supported list
    if (!MNList %in% supportedMNList) {
        stop(paste("MN must be one of the following:", paste(supportedMNList, collapse = ", ")))
    }
    ...
    masterNCT <- effectivenessCalculations::createMasterNct(nctList) |>
        ...
        dplyr::select(-dplyr::all_of(setdiff(supportedMNList, c(MNList, "Energy"))))
    ...
    fortifiableFoodVehicle <- fortifiableFoodItemsDf |>
        ...
        dplyr::select(food_genus_id, food_vehicle_id, food_vehicle_name, fortifiable_portion, !!rlang::sym(method))
    ...
    if (metric == "AFE" | metric == "BOTH") {
        ...
        earThreshholds <- intakeThresholdsDf |>
            ...
            tidyr::pivot_wider(names_from = nutrient, values_from = c(EAR, UL)) |>
    ...
    enrichedHouseholdConsumption <- householdConsumptionDf |>
        ...
        dplyr::mutate(dplyr::across(c("amountConsumedInG", "afeFactor", "fortifiable_portion", "memberCount", "Energy", MNList), as.numeric)) |>
        # TODO: Deal with the deprecation warning for external vectors here and switch to all_of
        dplyr::mutate(dplyr::across(MNList, ~ . / afeFactor, .names = "{.col}_perAFE"), amountConsumedInGAfe = amountConsumedInG / afeFactor) |>
    ...
    if (metric == "AFE" | metric == "BOTH") {
        for (nutrient in MNList) {
            if (grepl("mg", effectivenessCalculations::getMnThresholdUnits(intakeThresholdsDf, nutrient, "unitAdequacy"))) {
                ...
                for (year in years) {
                    ...
                    enrichedHouseholdConsumption[paste0(nutrient, "_", year, "_", method, "Supply")] <- enrichedHouseholdConsumption["amountConsumedInGAfe"] * (yearAverageFortificationLevel(fortification_vehicle = foodVehicleName, Year = year, MN = nutrient, fortificationLevels = fortificationLevelsDf) / 1000) * (enrichedHouseholdConsumption["fortifiable_portion"] / 100)
                }
                ...
                if (nutrient != "B9") {
                    ...
                    for (year in years) {
                        # Calculate the supply of the nutrient with LSFF per food item
                        # TODO: Check if this calculation is correct when all foods are tagged as fortifiable.
                        enrichedHouseholdConsumption[paste0(nutrient, "_", year, "_", method, "Supply")] <- enrichedHouseholdConsumption["amountConsumedInGAfe"] * yearAverageFortificationLevel(fortification_vehicle = foodVehicleName, Year = year, MN = nutrient, fortificationLevels = fortificationLevelsDf) * (enrichedHouseholdConsumption["fortifiable_portion"] / 100)
                    }
                } else {
                    enrichedHouseholdConsumption[paste0(nutrient, "_BaseSupply")] <- enrichedHouseholdConsumption[nutrient] / 100 * enrichedHouseholdConsumption["amountConsumedInG"]
                    # TODO: This is verified as correct. Line 112 divided the nuttrient by AFE factor hence we use amountConsumedInG and not amountConsumedInGAfe here. It's quacky I know but it's correct.
                    ...
                    for (year in years) {
                        # Calculate the supply of the nutrient with LSFF per food item
                        # TODO: Check if this calculation is correct when all foods are tagged as fortifiable.
                        enrichedHouseholdConsumption[paste0(nutrient, "_", year, "_", method, "Supply")] <- enrichedHouseholdConsumption["amountConsumedInGAfe"] * (yearAverageFortificationLevel(fortification_vehicle = foodVehicleName, Year = year, MN = nutrient, fortificationLevels = fortificationLevelsDf) * 1.7) * (enrichedHouseholdConsumption["fortifiable_portion"] / 100)
                    }
                }
            }
        }
        ...
        nutrientSupply <- enrichedHouseholdConsumption |>
            ...
            dplyr::summarize(
                ...
                dplyr::across(dplyr::ends_with(paste0("_", method, "Supply")), ~ sum(.x, na.rm = TRUE), .names = "{.col}")
            )
        ...
        for (nutrient in MNList) {
            for (year in years) {
                nutrientSupply[paste0(nutrient, "_", year, "_BaseAnd", method, "TotalSupply")] <- nutrientSupply[paste0(nutrient, "_BaseSupply")] + nutrientSupply[paste0(nutrient, "_", year, "_", method, "Supply")]
            }
        }
        ...
        for (nutrient in MNList) {
            ...
            for (year in years) {
                if (!is.na(effectivenessCalculations::getMnThresholds(intakeThresholdsDf, nutrient, "ear"))) {
                    enrichedNutrientSupply[paste0(nutrient, "_", year, "_base_and_", method_lower, "_ear_inadequacy")] <- ifelse(enrichedNutrientSupply[paste0(nutrient, "_", year, "_BaseAnd", method, "TotalSupply")] >= effectivenessCalculations::getMnThresholds(intakeThresholdsDf, nutrient, "ear"), 0, 1)
                }
            }
        }
        ...
        for (nutrient in MNList) {
            ...
            for (year in years) {
                if (!is.na(effectivenessCalculations::getMnThresholds(intakeThresholdsDf, nutrient, "ul"))) {
                    enrichedNutrientSupply[paste0(nutrient, "_", year, "_base_and_", method_lower, "_ul_exceedance")] <- ifelse(enrichedNutrientSupply[paste0(nutrient, "_", year, "_BaseAnd", method, "TotalSupply")] > effectivenessCalculations::getMnThresholds(intakeThresholdsDf, nutrient, "ul"), 1, 0)
                }
            }
        }
        ...
        for (nutrient in MNList) {
            for (year in years) {
                enrichedNutrientSupply[paste0(nutrient, "_", year, "_effective_coverage")] <- ifelse(enrichedNutrientSupply[paste0(nutrient, "_base_supply_ear_inadequacy")] != enrichedNutrientSupply[paste0(nutrient, "_", year, "_base_and_", method_lower, "_ear_inadequacy")], 1, 0)
            }
        }
    ...
    if (metric == "CND" | metric == "BOTH") {
        ...
        for (nutrient in MNList) {
            for (year in years) {
                if (grepl("mg", effectivenessCalculations::getMnThresholdUnits(intakeThresholdsDf, nutrient, "unitAdequacy"))) {
                    # Calculate the supply of the nutrient with LSFF per food item
                    enrichedHouseholdConsumption[paste0(nutrient, "_", year, "_LSFFSupply")] <- enrichedHouseholdConsumption["amountConsumedInG"] * (yearAverageFortificationLevel(fortification_vehicle = foodVehicleName, Year = year, MN = nutrient, fortificationLevels = fortificationLevelsDf) / 1000) * enrichedHouseholdConsumption["fortifiable_portion"] / 100
                } else if (grepl("mcg", effectivenessCalculations::getMnThresholdUnits(intakeThresholdsDf, nutrient, "unitAdequacy"))) {
                    if (nutrient != "B9") {
                        # Calculate the supply of the nutrient with LSFF per food item
                        ...
                        # Calculate the supply of the nutrient with LSFF per food item
                        enrichedHouseholdConsumption[paste0(nutrient, "_", year, "_LSFFSupply")] <- enrichedHouseholdConsumption["amountConsumedInG"] * (yearAverageFortificationLevel(fortification_vehicle = foodVehicleName, Year = year, MN = nutrient, fortificationLevels = fortificationLevelsDf) * 1.7) * enrichedHouseholdConsumption["fortifiable_portion"] / 100
                    }
                }
